...
(function () {
  ...
  const POOL_KEY = "muxu_bd_pool"; // 二次创作候选池（localStorage 持久化）
  const FLOW_STEPS = [
    "① 输入热点",
    "② 去重排序",
    "③ 跟进判断",
  ...
  function renderFlow(step) {
    STATE.step = step;
    $("bdFlow").innerHTML = FLOW_STEPS.map((s, i) => {
      const n = i + 1;
  ...
  function renderDedupe() {
    const hot = STATE.hot;
    $("bdDedupTag").textContent = `共 ${hot.length} 条 · 保留 ${hot.filter((h) => !h.dup).length} · 合并重复 ${hot.filter((h) => h.dup).length}`;
    $("bdDedupBody").innerHTML = hot.map((h) => {
      const dupHtml = h.dup
  ...
  function renderJudged() {
    $("bdJudged").innerHTML = STATE.judged.map((j, i) => {
      const h = j.hot;
      ...
      return `<div class="judge-card">
        ...
        <div class="bd-field" style="margin-top:8px;"><span class="k">跟进判断理由（可编辑）</span>
          <textarea id="bdReason${i}" style="min-height:44px;">${esc(j.reason)}</textarea></div>
      </div>`;
    }).join("");
  }
  ...
  function genCandidates() {
    const cat = ($("bdCategory").value || "").trim();
    ...
    $("bdCandList").innerHTML = STATE.cand.map((c, i) => {
      ...
      return `<div class="card">
        ...
        <div class="tweet"><span class="lang">🇨🇳 中文候选（可编辑）</span>
          <textarea id="bdCandCn${i}" style="width:100%;min-height:64px;border:1px solid var(--border);border-radius:8px;padding:8px 10px;font-size:13px;font-family:inherit;resize:vertical;background:#fff;margin-top:6px;">${esc(c.cn)}</textarea>
          <button class="copy" data-copy-i="${i}" data-lang="cn">复制</button></div>
        <div class="tweet"><span class="lang">🌐 英文候选（可编辑）</span>
          <textarea id="bdCandEn${i}" style="width:100%;min-height:64px;border:1px solid var(--border);border-radius:8px;padding:8px 10px;font-size:13px;font-family:inherit;resize:vertical;background:#fff;margin-top:6px;">${esc(c.en)}</textarea>
          <button class="copy" data-copy-i="${i}" data-lang="en">复制</button></div>
      </div>`;
    }).join("") || `<p class="note">没有「适合跟进」的热点，请返回上一步调整账号领域或热点列表。</p>`;
    $("bdGenHint").textContent = `已为 ${STATE.cand.length} 个热点生成中英文候选，可逐条编辑后存入待审核池。`;
  }
  ...
  function copyCand(i, lang) {
    const el = $("bdCand" + (lang === "en" ? "En" : "Cn") + i);
  ...
  function saveToPool() {
    ...
    STATE.cand.forEach((c, i) => {
      ...
      const base = {
        ...
        reason: $("bdReason" + STATE.judged.indexOf(j)) ? $("bdReason" + STATE.judged.indexOf(j)).value : j.reason,
      ...
      if (ex) {
        ...
        pool.push({
          id: "BD_" + today().replace(/-/g, "") + "_" + (pool.length + 1) + "_" + Date.now().toString(36).slice(-3),
          ...base,
    ...
    savePool(pool);
    $("bdSaveHint").textContent = `✅ 已存入待审核池：新增 ${added} 条${updated ? "，更新 " + updated + " 条" : ""}。请在下方「待审核内容池」逐条采纳/驳回。`;
    renderFlow(5);
    renderPool();
    renderStrategy();
  ...
  function renderPool() {
    ...
    $("bdPoolBody").innerHTML = shown.map((p) => {
      const st = p.status;
      return `<div class="card">
        <div class="chead">
  ...
  function buildStrategy() {
    ...
    L.push("【二次创作优化策略】（基于 " + pool.length + " 条候选的审核反馈自动生成）");
    L.push("");
  ...
  function exportPool() {
    ...
    a.download = `二次创作候选池_${today()}.json`;
    a.click();
    toast("已导出候选池 JSON");
  }

  // ---------- 绑定 ----------
  $("bdImportData").addEventListener("click", importFromData);
  $("bdLoadSample").addEventListener("click", loadDemo);

  $("bdRun").addEventListener("click", function () {
    const items = parseInput($("bdHotInput").value);
    if (!items.length) {
      $("bdRunHint").textContent = "⚠️ 请先输入热点（或点击「从今日热点导入」/「载入 Demo 示例」）";
      return;
    }
    dedupeAndSort(items);
    renderDedupe();
    ...
    $("bdSec3").hidden = true;
    renderFlow(2);
    $("bdRunHint").textContent = `已解析 ${items.length} 条热点，去重后保留 ${STATE.hot.filter((h) => !h.dup).length} 条。`;
    toast("去重排序完成");
  });

  $("bdJudge").addEventListener("click", function () {
    const cat = ($("bdCategory").value || "").trim();
    if (!cat) {
      $("bdJudgeHint").textContent = "⚠️ 请先填写「账号领域」，判定才有依据";
      return;
    }
    ...
    $("bdSec2").hidden = false;
    renderFlow(3);
    $("bdJudgeHint").textContent = `已判定 ${STATE.judged.length} 条：适合 ${STATE.judged.filter((j) => j.fit).length} 条。`;
    toast("跟进判断完成");
  });

  $("bdGen").addEventListener("click", function () {
    genCandidates();
    $("bdSec3").hidden = false;
    renderFlow(4);
    toast("已生成中英文候选");
  ...
  // 候选编辑 + 复制（事件委托）
  $("bdCandList").addEventListener("input", function (e) {
    const m = /^bdCand(Cn|En)(\d+)$/.exec(e.target.id || "");
    if (m) syncCandEdit(+m[2], m[1] === "En" ? "en" : "cn", e.target.value);
  });
  $("bdCandList").addEventListener("click", function (e) {
    if (e.target.classList.contains("copy")) {
      copyCand(+e.target.dataset.copyI, e.target.dataset.lang);
    }
  });

  $("bdSavePool").addEventListener("click", saveToPool);

  // 池过滤 + 采纳/驳回 + 备注（事件委托）
  $("bdPoolFilters").addEventListener("click", function (e) {
    if (e.target.dataset.f) {
      poolFilter = e.target.dataset.f;
      renderPool();
    }
  });
  $("bdPoolBody").addEventListener("click", function (e) {
    if (e.target.classList.contains("copy")) {
  ...
  $("bdPoolBody").addEventListener("input", function (e) {
    if (e.target.dataset.note) notePool(e.target.dataset.note, e.target.value);
  });

  $("bdCopyStrategy").addEventListener("click", function () {
    copyText($("bdStrategy").value, "已复制优化策略");
  });
  $("bdExportPool").addEventListener("click", exportPool);
